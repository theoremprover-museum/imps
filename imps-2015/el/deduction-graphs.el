...
\\{sqn-mode-map}\n"
  ...
  (make-variable-buffer-local 'mode-line-filler)
...
(defun dg-xview-sqn (sqn-no)
  ...
  (tea-eval-expression
   (format
    "(xview (sequent-unhash %d))" sqn-no)))

(defun dg-xview-dg ()
  "Run xview on the Current DG."
  (message "Starting xview on deduction graph.")
  (tea-eval-expression (format "(xview (current-dg))")))
...
(defun dg-update-sqn-and-dg (verbose)
  ...
  (tea-eval-large-expression
   (format "(block (emacs-add-all-new-sqns (current-dg))
		   (emacs-%supdate-dg (current-dg)))"
	   (if verbose "verbose-" ""))))  
...
if interactive) means put in all parentheses.  "
  ...
  (let ((sqn-index (current-sqn-no)))
    ...
    (tea-eval-expression
     (format
      "(emacs-send-sqn-to-edit (sequent-unhash %d) %s)"
      sqn-index
      (if fully-parenthesize "(true)" "(false)")))))
...
(defun imps-start-deduction-internal (formula)    
  (tea-eval-large-expression
   (format "(clear-em) (start-emacs-deduction %s)"
   ...
   (format
    "(if (deduction-graph-history (current-dg))
         (set (dg-history-entry-comments 
              (car (deduction-graph-history (current-dg)))) (list \"%s\"))"
