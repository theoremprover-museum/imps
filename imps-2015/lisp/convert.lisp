...
(defun translate-source-file (source-dir target-dir file before after)
  (setf *print-case* :downcase)
  (format 't "PROCESSING FILE ~A~%" file)
  (format logport "PROCESSING FILE ~A~%~%" file)
  (with-open-file 
   (current-in (truename (format nil "~A/~A.~A" source-dir file before)) :direction :input)
   (with-open-file 
    (current-out (truename (format nil "~A/~A.~A" target-dir file after)) :direction :output)
    (write  *header-string* :stream current-out :escape nil)
    (format  current-out "~%")
    (pprint *package-form* current-out)
    (format  current-out "~%")
    (loop
     (let ((sexp (read current-in nil 'eof-value)))
	   ...
	   (progn (close current-in)
		  (close current-out)
		  (return-from translate-source-file))
	 (progn
	   (pprint (convert-top-level sexp) current-out)
	   (format  current-out "~%"))))))))

(defun convert-source-files (source-dir target-dir the-files before after)
  (let ((notifications nil))
    (with-open-file 
     (logport (truename logfile) :direction :output)
     (mapc #'(lambda (x) (translate-source-file source-dir target-dir (string-downcase (format nil "~A" x)) before after)) the-files)
     (format logport "~A Notification~P" (length notifications) (length notifications))
     ))
  'convert-done)
...
;;;
;;;
;;;(defun file-structure-accessors (source-dir file type target-port)
;;;  (setf *print-case* :downcase)
;;;  (format 't "PROCESSING FILE ~A~%" file)
;;;  (with-open-file 
;;;   (current-in (truename (format nil "~A/~A.~A" source-dir file type)) 
;;;	       :direction :input)
;;;   (loop
;;;     (let ((sexp (read current-in nil 'eof-value)))
;;;       (if (eql sexp 'eof-value)
;;;	   (progn (close current-in)
;;;		  (return-from file-structure-accessors))
;;;	   (scan-deftype sexp target-port))))))
;;;
;;;(defun imps-structure-accessors (the-files)
;;;  (with-open-file 
;;;   (current-out (truename accessor-list-file) :direction :output)
;;;   (pprint *package-form* current-out)
;;;   (format current-out "~%")
;;;   (mapc #'(lambda (x) 
;;;	     (file-structure-accessors 
;;;	      source-dir 
;;;	      (string-downcase (format nil "~A/~A" (car x) (cadr x)))
;;;	      "t"
;;;	      current-out))
;;;	 the-files))
;;;  'done)
...
;;;
;;;
;;;(defun scan-deftype (expr current-out)
;;;  (if (eq (car expr) 'define-structure-type)
;;;      (let ((fields-handlers (cdr expr))
;;;	    (fields nil) 
;;;	    (handlers nil))
;;;	(if (listp (car (last fields-handlers)))
...
;;;	       ',(mapcar #'(lambda (x) (concatenate-symbol (car fields) '- x))
;;;			 (cdr fields)))
;;;	 current-out)
;;;	(format current-out "~%"))))
