...
(defun setf-value-and-closure (obj)
  (let ((the-name (object-id obj)))
    (if (symbolp the-name)
	(progn
	  (proclaim (list 'special the-name))
	  (setf (symbol-function the-name) (object-closure obj))
	  (setf (symbol-value the-name) obj))
      (error "~A is not a symbol" the-name))
    obj))
...
(defun build-operation (the-name default handlers)
  (or (symbolp the-name) (error "BUILD-OPERATION: ~A is not a symbol" the-name))
  (if (null the-name) (error "BUILD-OPERATION: ~A invalid name" the-name))
...
(defun build-settable-operation (the-name default)
  (let ((setter-op (make-operation-init)))
     (build-operation the-name default
       (vector (vector *setter* (function (lambda (soi) (declare (ignore soi)) setter-op)))))))

(defun build-predicate (the-name)
  (build-operation the-name (function (lambda (x) (declare (ignore x)) '())) nil))

(defun build-switch (the-name init-value predicate)
  (let ((switch 
	 (make-switch-init 
	  :id the-name 
	  :state init-value 
...
;(defmacro def-object-setf (the-name lambda-list store-value)
;  `(defsetf-1 ,the-name ,lambda-list (,store-value)
;     (list 'tea::funcall '(setter (symbol-value ',the-name))
...
(defmacro tea::define-operation (&rest args)
  (cond ((symbolp (car args))
	 (let ((the-name (car args))
	       (vars (list '&rest (gensym))))
	   (or (= (length args) 1)
	       (error "Bad syntax ~A:" args))
	   `(progn
	     (setf-value-and-closure 
	     (build-operation ',the-name
			      (tea::lambda ,vars (error "~A: No default method for operation." ',the-name))
			      nil)))))

	((symbolp (caar args))
	 (destructure 
	  ((((the-name . vars) . body) args))
	  `(progn
	    (setf-value-and-closure 
	    (build-operation ',the-name
	...
	('t (destructure (((((the-name . vars) . body) . handlers) args))
			 `(progn
			   (setf-value-and-closure 
			   (build-operation ',the-name 
...
(defmacro tea::define-simple-switch (the-name predicate init-value)
  (let ((the-name (if (symbolp the-name) 
		  the-name
		(car the-name))))
  `(progn
     (setf-value-and-closure
      (build-switch ',the-name ,init-value ,predicate))
     ;(def-object-setf ,the-name nil ,(gensym))
     )))
  
;;ALmost the same. For compatability.

(defun tea::make-simple-switch (the-name predicate &optional init-value)
  (build-switch the-name init-value predicate))

(defmacro tea::define-predicate (the-name)
  `(progn (setf-value-and-closure (build-predicate ',the-name))))

(defmacro tea::define-settable-operation ((the-name . vars) . body)
  `(progn
     (setf-value-and-closure
      (build-settable-operation ',the-name 
			       (tea::lambda ,vars ,@(or body '(*operation-default-value*)))))
     ;(def-object-setf ,the-name ,vars ,(gensym))
     ))
...
(setf-value-and-closure
 (build-operation 'tea::print-type-string 
		  (function (lambda (thing) (declare (ignore thing)) "Thing"))
		  nil))
