...
(defmacro tea::define-structure-type (name &rest fields-handlers)
  (let ((fields nil) 
	(handlers nil) 
	(raw-constructor-name (gentemp (concatenate 'string (symbol-name name) "-RAW-CONSTRUCTOR-")))
	(constructor-name (concatenate-symbol  'make- name))
	(copier-name (gentemp (concatenate 'string (symbol-name name) "-COPIER-" )))
	(predicate-name (concatenate-symbol name '?))
	(attribute-vector-name (concatenate-symbol name '-attributes))
	(handler-vector-name (concatenate-symbol name '-handlers))
	(stype-symbol (concatenate-symbol name '-stype))
	(n 0))
